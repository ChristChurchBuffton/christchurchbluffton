...
## STATUS (2026-08-13): Admin clean-URL fix + new public-site logo (loader/header/footer) + Greeters mass-upload — ALL LOCALHOST ONLY except the Greeters DB write, nothing pushed to git yet
...
- [ ] **Not pushed** — Kevin's explicit "localhost only" instruction for this whole batch. Netlify's `pretty_urls` (already on in `netlify.toml`) handles the same rewrite for real `<a href>` tags in production automatically; this fix makes the admin's own JS-driven redirects and sidebar links match that behavior too.
...
- [ ] **Nothing pushed** — all logo/CSS work above is local-file-only, confirmed multiple times against Kevin's "localhost only" instruction.
