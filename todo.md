...
## Queued for next push (2026-08-11 addition)
- [ ] `src/admin/subscribers.html` — Subscribers list now dedupes by email for display. Public forms still don't block someone from submitting the newsletter/contact form more than once (unchanged, multiple raw rows still get written), but the admin Subscribers list and its "X people" count now collapse those to one row per email (the most recent submission, since the underlying data loads newest-first). Editing/removing that row only affects that one raw record — if older duplicate rows exist for the same email, the next one surfaces after the top one is removed. Not yet live-tested or pushed.
...
- [x] All three forms now have auto-replies wired in (prayer, newsletter, contact) — live-tested on staging 2026-08-11: dedicated prayer form, dedicated newsletter form, contact form with each of "Other", "Prayer Request" (x2), and "Receiving Updates" (x2) all submitted with a real test address, staff notifications temporarily disabled during the test. Kevin confirmed results looked right.
- [x] Staff notifications (admin@/jonathan@/bradley@/info@) restored/uncommented in all three functions 2026-08-11 — verified no leftover TEMP markers, syntax-checked clean. Ready for a final push to `origin` then `production`.
